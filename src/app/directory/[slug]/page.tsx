...
import { Star, MapPin, Globe, Mail, Phone, ArrowLeft } from "lucide-react";
...
export default function BusinessProfilePage({ params }: Props) {
  ...
  // Fetch reviews once business is loaded
  useEffect(() => {
    if (!business) return;
    supabase
      .from("reviews")
  ...
  const handleReviewSubmit = async (e: React.FormEvent) => {
    ...
    // Refresh reviews — trigger already updated the business rating in DB
    ...
    if (fresh) {
      setReviews(fresh);
      setUserReview(fresh.find((r) => r.user_id === currentUser.id) ?? null);
    }

    // Refresh business rating shown on page
    ...
    if (updatedBiz) {
      setBusiness((prev) => prev ? { ...prev, ...updatedBiz } : prev);
    }
            ...
            <div className="bg-white rounded-2xl p-8 shadow-card">
                    ...
                    <div className="flex gap-0.5">
                      {[1, 2, 3, 4, 5].map((n) => (
                        <Star
                          key={n}
                          size={14}
                          className={n <= Math.round(business.rating) ? "fill-gold-500 text-gold-500" : "fill-transparent text-slate-200"}
                        />
                      ))}
                    </div>
              ...
              {!currentUser ? (
                <div className="mb-8 p-4 rounded-xl bg-forest-50 border border-forest-100 text-center">
                  ...
                  <Link
                    href="/auth"
                    className="inline-flex px-5 py-2 rounded-lg bg-forest-800 text-white text-sm font-semibold hover:bg-forest-700 transition-colors"
                  >
                    Sign In
                  </Link>
                </div>
                ...
                <div className="mb-8 p-4 rounded-xl bg-green-50 border border-green-100">
                  ...
                  <div className="flex gap-0.5 mb-1">
                    {[1, 2, 3, 4, 5].map((n) => (
                      <Star key={n} size={13} className={n <= userReview.rating ? "fill-gold-500 text-gold-500" : "fill-transparent text-slate-300"} />
                    ))}
                  </div>
                ...
                <form onSubmit={handleReviewSubmit} className="mb-8 p-5 rounded-xl bg-slate-50 border border-slate-100 space-y-4">
                  ...
                  {reviewSuccess && (
                    <p className="text-xs text-green-600">Review submitted — thank you!</p>
                  )}
                  ...
                  {reviews.map((review, i) => (
                    <div key={review.id} className={i > 0 ? "pt-6 border-t border-forest-50" : ""}>
                      ...
                      <div className="flex gap-0.5 mb-2">
                        {[1, 2, 3, 4, 5].map((n) => (
                          <Star key={n} size={12} className={n <= review.rating ? "fill-gold-500 text-gold-500" : "fill-transparent text-slate-200"} />
                        ))}
                      </div>
